...
use VoyagerAlbums\Models\AlbumsImage;
...
// Check permission
    ...
    protected function cleanup($dataType, $data)
    {
        ...
        foreach ($dataType->deleteRows->where('type', 'file') as $row) {
            $files = json_decode(json_encode($data->{$row->field}),true);
            if ($files) {
        ...
        foreach ($rows as $row) {
            ...
            if($edit) {
                if ($row->type=="text_area"){
                    $data=AlbumsImage::whereid($request->id)->firstOrFail();
                    $input = $this->getContentBsdOnType($request, $slug, $row);
                    $data->name = $input;
                    $data->save();
                }
            }
        ...
        switch ($row->type) {
            ...
            case 'multiple_images':
                if ($files = $request->file($row->field)) {
                    ...
                    foreach ($files as $key => $file) {
                        $filename = Str::random(20);
                        $path = $slug.'/'.date('FY').'/'.$request->voyager_albums_id.'/';
